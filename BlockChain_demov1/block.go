...
type Block struct {
	...
	Difficulty    uint64 //通过这个数字，算出哈希值：0x00010000000xxx
	Nonce         uint64 //这是我们要找的随机数，挖矿就找证书
	Hash          []byte //当前区块哈希值，正常的区块不存在，我们为了方便放进来的
	Data          []byte //数据本身，区块体，先用字符串表示，v4版本的时候会引用真正交易结构
}
